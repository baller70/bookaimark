...
const InputTabs: React.FC = () => {
  ...
  const handleSingleUrlSubmit = useCallback(async () => {
    ...
    try {
      ...
      // Add to bulk uploader state
      dispatch({ type: 'SET_LINKS', payload: [...state.links, newLink] });

      // Also save directly to bookmarks API
      const bookmarkData = {
        title: singleTitle.trim() || singleUrl.trim(),
        url: singleUrl.trim(),
        description: singleNotes.trim() || `Bookmark saved from bulk uploader`,
        category: newLink.predictedFolder || 'General',
        tags: newLink.predictedTags,
      ...
      // Update the link status to saved
      dispatch({ 
        type: 'UPDATE_LINK', 
        payload: { 
          id: newLink.id, 
      ...
      toast.success('✨ URL saved to bookmarks successfully!');
      ...
      console.error('Error saving single URL:', error);
