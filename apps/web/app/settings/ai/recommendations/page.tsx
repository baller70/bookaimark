...
const CombinedProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    ;(async () => {
      ...
      if (user) {
        try {
          const [recSettings, finderSettings] = await Promise.all([
            getAISetting(user.id, 'recommendations').catch(() => defaultRecommendationSettings),
            getAISetting(user.id, 'link_finder').catch(() => defaultLinkFinderPrefs)
          ])
          
          const combined = {
            recommendations: recSettings,
            linkFinder: finderSettings
          }
  ...
  const saveSettings = async () => {
    ...
    if (user) {
      try {
        await Promise.all([
          saveAISetting(user.id, 'recommendations', settings.recommendations),
          saveAISetting(user.id, 'link_finder', settings.linkFinder)
        ])
          ...
          <div className="space-y-4">
            ...
            <div>
              <Label className="text-sm font-medium mb-3 block">
                Max links: {settings.linkFinder.maxLinks}
              </Label>
              <Slider
                value={[settings.linkFinder.maxLinks]}
                onValueChange={([value]) => updateLinkFinderSetting('maxLinks', value as LinkFinderPrefs['maxLinks'])}
                max={100}
                min={10}
                step={15}
                className="w-full"
