      ...
      try {
        ...
        if (content || url) {
          try {
            ...
            logger.warn('Failed to get AI analysis for tagging', {
              url,
        ...
        if (finalOptions.categoryWeighting && analysis) {
          this.applyCategoryWeighting(suggestions, analysis.category);
        }
        
        // Deduplicate and rank suggestions
        const rankedTags = this.rankAndDeduplicateTags(suggestions, finalOptions);
        
        logger.info('Intelligent tags generated', {
          url,
          tagCount: rankedTags.length,
          topTags: rankedTags.slice(0, 5).map(t => t.tag)
        });
...
//     return await performanceUtils.trackFunction('tag_improvement_suggestions', async () => {
  ...
  private extractAiTags(analysis: ContentAnalysisResult, options: TaggingOptions): TagSuggestion[] {
    ...
    analysis.tags.forEach(tag => {
      if (this.isValidTag(tag, options)) {
        suggestions.push({
          ...
          relatedTags: analysis.relatedKeywords.slice(0, 3)
        });
      }
    });
    ...
    if (analysis.category && analysis.category !== 'other') {
      suggestions.push({
        tag: analysis.category.toLowerCase(),
        confidence: 0.95,
        source: 'ai',
        category: analysis.category
      });
