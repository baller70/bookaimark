...
CREATE INDEX IF NOT EXISTS idx_user_bookmarks_updated_at ON public.user_bookmarks(updated_at DESC);
CREATE INDEX IF NOT EXISTS idx_user_bookmarks_is_favorite ON public.user_bookmarks(is_favorite);
CREATE INDEX IF NOT EXISTS idx_user_bookmarks_folder_id ON public.user_bookmarks(folder_id);
CREATE INDEX IF NOT EXISTS idx_user_bookmarks_priority ON public.user_bookmarks(priority);
CREATE INDEX IF NOT EXISTS idx_user_bookmarks_link_type ON public.user_bookmarks(link_type);
CREATE INDEX IF NOT EXISTS idx_user_bookmarks_language ON public.user_bookmarks(language);
...
RETURNS TRIGGER AS $$
BEGIN
    INSERT INTO public.user_bookmark_folders (user_id, name, description, color, is_system_folder)
    VALUES 
        (NEW.id, 'Work', 'Work-related bookmarks', '#3B82F6', true),
        (NEW.id, 'Personal', 'Personal bookmarks', '#10B981', true),
        (NEW.id, 'Learning', 'Educational resources', '#F59E0B', true),
        (NEW.id, 'Tools', 'Useful tools and utilities', '#8B5CF6', true),
        (NEW.id, 'Inspiration', 'Design and creative inspiration', '#EF4444', true)
...
CREATE TRIGGER create_default_bookmark_folders_trigger
    AFTER INSERT ON auth.users
    FOR EACH ROW
    EXECUTE FUNCTION create_default_bookmark_folders();
...
CREATE OR REPLACE VIEW public.user_bookmarks_with_folders AS
SELECT 
    b.*,
    f.name as folder_name,
    f.color as folder_color
FROM public.user_bookmarks b
LEFT JOIN public.user_bookmark_folders f ON b.folder_id = f.id;

-- Grant access to the view
GRANT SELECT ON public.user_bookmarks_with_folders TO authenticated;

-- Comments for documentation
COMMENT ON TABLE public.user_bookmarks IS 'Stores user bookmarks with AI-powered categorization and metadata';
COMMENT ON TABLE public.user_bookmark_folders IS 'Stores user-created bookmark folders for organization';
COMMENT ON COLUMN public.user_bookmarks.ai_summary IS 'AI-generated summary of the bookmark content';
COMMENT ON COLUMN public.user_bookmarks.ai_tags IS 'AI-generated tags for the bookmark';
COMMENT ON COLUMN public.user_bookmarks.ai_category IS 'AI-predicted category for the bookmark';
COMMENT ON COLUMN public.user_bookmarks.reading_time_minutes IS 'Estimated reading time in minutes';
COMMENT ON COLUMN public.user_bookmarks.word_count IS 'Word count of the page content';
COMMENT ON COLUMN public.user_bookmarks.language IS 'Detected language of the content'; 