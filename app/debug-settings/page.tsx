...
export default function DebugSettings() {
  ...
  useEffect(() => {
    ...
    const collectDebugInfo = async () => {
      ...
      try {
        ...
        info.supabase.session = session ? 'exists' : 'no_session'
        info.supabase.user = session?.user?.id || 'no_user'
      } catch (error) {
        info.errors.push({ type: 'supabase_session', error: (error as Error).message })
      }
      ...
      try {
        ...
        info.errors.push({ type: 'api_test', error: (error as Error).message })
      }
  ...
  const sendTestNotification = async (channel: string) => {
    return Sentry.startSpan(
      {
        op: "ui.click",
        name: `Debug Test ${channel} Notification`,
      },
      async (span) => {
        try {
          ...
          if (channel === 'push') {
            ...
            if ('Notification' in window) {
              ...
              if (permission === 'granted') {
                ...
                span.setAttribute("notification_created", true)
              } else {
                console.log('🔔 DEBUG: Permission denied')
                toast.error('Push notification permission denied')
                span.setAttribute("permission_denied", true)
              }
            } else {
              console.log('🔔 DEBUG: Notifications not supported')
              toast.error('Push notifications not supported in this browser')
              span.setAttribute("not_supported", true)
            }
          } else {
            ...
            span.setAttribute("api_success", result.success)
