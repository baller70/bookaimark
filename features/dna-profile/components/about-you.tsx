...
export function AboutYouComponent({ onProfileUpdate }: AboutYouComponentProps) {
    ...
    const checkAuthStatus = async () => {
      if (!supabase) {
        toast({
          ...
          description: "Supabase not configured. Your profile will be saved locally.",
        })
        return
      }
      
      // Show message about bypass mode
      toast({
        title: "Testing mode active",
        description: "Authentication bypassed. Data will be saved to Supabase with test user ID.",
      })
    }
    
    // Delay the auth check to avoid interfering with page load
  ...
  const loadProfile = async () => {
    try {
      if (!supabase) {
        // Check for locally saved profile first
        ...
        // Use default profile if no local data
        console.log('Loading profile with default data')
        setProfile(defaultProfile)
        return
      }

      // Try to load from Supabase using mock user ID
      try {
        ...
        console.warn('Supabase query failed, checking local storage:', supabaseError)
      ...
      console.warn('Error in loadProfile:', error)
      setProfile(defaultProfile)
    } finally {
      setLoading(false)
    }
  ...
  const saveProfile = async () => {
    ...
    try {
      ...
      // Use mock user ID for testing (bypass authentication)
      const profileData = {
        ...profile,
      ...
      // Save to Supabase using mock user ID
      ...
      if (error) {
        ...
        toast({
          title: "Profile saved locally",
          description: "Could not save to Supabase. Profile saved locally instead.",
        })
      ...
      toast({
        title: "Profile saved to Supabase!",
        description: "Your DNA profile has been updated in the database.",
      })
      ...
      try {
        ...
        toast({
          title: "Profile saved locally",
          description: "Saved to this device. Could not reach Supabase.",
        })
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // This bypasses authentication and ensures the avatar is always saved
    ...
    reader.onload = (event) => {
      ...
      const userSettings = JSON.parse(localStorage.getItem('userSettings') || '{}')
          ...
          toast({
            ...
            description: "Your profile picture has been saved and will appear on bookmark cards.",
          })
        } catch (storageError) {
        ...
        // Try to upload to Supabase if user is authenticated (optional)
  ...
  const trySupabaseUpload = async (file: File) => {
    try {
      // Skip authentication entirely - just use localStorage and file URLs
      console.log('Supabase upload bypassed - using localStorage only for profile pictures')
      console.log('File size:', file.size, 'File type:', file.type)
      
      // Could add other storage methods here if needed (like Cloudinary, etc.)
      // For now, localStorage provides immediate functionality without authentication
      
    } catch (error) {
      console.log('Background upload skipped:', error)
    }
  }
